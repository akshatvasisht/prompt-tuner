...
describe("ACTIONS Registry", () => {
  ...
  it("should have exactly 6 secondary actions", () => {
    const secondaryActions = ACTIONS.filter((a) => a.type === "secondary");
    expect(secondaryActions.length).toBe(6);
  });

  it("should have no duplicate IDs when actions are registered", () => {
    const ids = ACTIONS.map((a) => a.id);
    const uniqueIds = new Set(ids);
    expect(uniqueIds.size).toBe(ids.length);
  });

  it("should have valid properties for each action when the registry is loaded", () => {
