...
- Ensure at least **22 GB free disk space** on your Chrome profile volume. Gemini Nano is ~2 GB, but Chrome purges the model automatically if free space drops below 10 GB.
- Trigger the download manually at `chrome://components/` once disk space is available.
