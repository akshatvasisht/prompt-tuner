...
import { type ScrapedContent, type OptimizationRule, type GeminiResponse } from "./types.js"
 ...
 */
async function distillContent(scraped: ScrapedContent): Promise<OptimizationRule[]> {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (jsonFiles.length === 0) {
      // eslint-disable-next-line no-console
      console.log("No scraped content found. Run `npm run pipeline:scrape` first.")
      return
    }
    ...
    for (const [platform, rules] of Object.entries(allRules)) {
      if (rules.length > 0) {
        ...
        // eslint-disable-next-line no-console
        console.log(`  [OK] Saved ${String(rules.length)} rules to ${platform}.json`)
      }
    }
