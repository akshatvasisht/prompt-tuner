...
Tests MiniPill trigger injection on supported LLM platforms.
...
Tests the complete prompt optimization workflow using the command palette overlay. Tests are currently skipped pending overlay-aware E2E helpers.

**Coverage** (planned):

- User triggers command palette
- Action selection and streaming response display
- Text injection back to platform on accept
- Cancel without injection
